...
class GainRatio
{
	...
	{
		System.out.println("In Gainratio --- cuurNodeEntophy ");
		int currentindex=0;
		double entropy=0;
	...
	{ 
		...
		try {
			...
			config.set("fs.defaultFS", "my-remote-ip");
			config.set("hadoop.job.ugi", "hdfs");
			FileSystem dfs = FileSystem.get(config);

			Path path = new Path("C45/intermediate"+id.current_index+".txt");
			
			System.out.println("file exists:C45/intermediate"+id.current_index+".txt"+dfs.exists(path));
